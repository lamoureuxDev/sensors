...
The Sensor Alerts program is a UI, designed to display a sensors alert message when the current value is out of normal range value. The normal range value is setting in the init process, then compare the current value with the range.
...
A shared memory is used to communicate between processes and share sensors data. Sensor\+\_\+alerts use the shared memory only for reading purpose.
...
\DoxyCodeLine{\$\ qmake}
\DoxyCodeLine{\$\ make}
\DoxyCodeLine{\$\ ./sensor\_alerts}

\end{DoxyCode}
\doxysection{Deployment}\label{index_Deployment}
\doxysubsection{Linux}\label{index_Linux}
Follow these steps to deploy the Sensor\+\_\+alerts on your Linux system\+:
\begin{DoxyItemize}
...
Following theses steps, Sensor\+\_\+alerts program will run automaticaly on boot, along side the sensor\+\_\+simulator and Sensor\+\_\+stats, with the sensor\+\_\+simulator launched fisrt.\doxysubsection{Yocto}\label{index_Yocto}
...
To populate our recipe for sensor-\/alerts, we will import our source files in our recipe to get this architecture\+: (assuming that \$\+BUILD\+\_\+\+Sensor\+\_\+\+ALERTS= /path/to/\+Sensor\+\_\+alerts\+\_\+sources)


\begin{DoxyCode}{0}
\DoxyCodeLine{../poky/buils\$\ bitbake-\/layers\ create-\/layer\ ../meta-\/sensors}
\DoxyCodeLine{../poky/buils\$\ cd\ ../meta-\/sensors}
\DoxyCodeLine{../poky/meta-\/sensors\$\ mv\ recipes-\/example\ recipes-\/sensor-\/alerts}
\DoxyCodeLine{../poky/meta-\/sensors\$\ mkdir\ recipes-\/sensor-\/alerts/sensor-\/alerts}
\DoxyCodeLine{../poky/meta-\/sensors\$\ mkdir\ recipes-\/sensor-\/alerts/sensor-\/alerts/files}
\DoxyCodeLine{../poky/meta-\/sensors\$\ cp\ \$BUILD\_Sensor\_ALERTS\textcolor{comment}{/*.*\ recipes-\/sensor-\/alerts/sensor-\/alerts/files/}}
\DoxyCodeLine{\textcolor{comment}{...poky/meta-\/sensors\$\ tree\ recipes-\/sensor-\/alerts/\ -\/L\ 3}}
\DoxyCodeLine{\textcolor{comment}{recipes-\/sensor-\/alerts/}}
\DoxyCodeLine{\textcolor{comment}{└──\ sensor-\/alerts}}
...
\DoxyCodeLine{\textcolor{comment}{\ \ \ \ │\ \ \ ├──\ Sensor\_alerts.pro}}
\DoxyCodeLine{\textcolor{comment}{\ \ \ \ │\ \ \ ├──\ Sensor\_alerts.pro.user}}
...
We will edit our recipe sensor-\/alerts\+\_\+0.\+1.\+bb as follow \+:


\begin{DoxyCode}{0}
\DoxyCodeLine{SUMMARY\ =\ \textcolor{stringliteral}{"{}My\ Sensor\ Alert\ application"{}}}
...
\DoxyCodeLine{\textcolor{stringliteral}{\ \ \ \ \ \ \ \ \ \ \ file://sensor\_alerts.pro\ \(\backslash\)}}
\DoxyCodeLine{\textcolor{stringliteral}{\ \ \ \ \ \ \ \ \ \ \ file://sensoralert.cpp\ \(\backslash\)}}
\DoxyCodeLine{\textcolor{stringliteral}{\ \ \ \ \ \ \ \ \ \ \ file://sensoralert.h\ \(\backslash\)}}
\DoxyCodeLine{\textcolor{stringliteral}{\ \ \ \ \ \ \ \ \ \ \ file://sensoralert.ui\ "{}}}
\DoxyCodeLine{\ \ \ \ \ \ \ \ \ \ \ }
\DoxyCodeLine{}
...
\DoxyCodeLine{\ \ \ \ install\ -\/m\ 0755\ \$\{S\}/sensor-\/alerts\ \$\{D\}\$\{datadir\}/\$\{P\}}
