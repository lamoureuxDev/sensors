...
The Sensor Simulator program is a UI, designed to set random sensors values in a shared memory.
Values are setted every second by default with a  different fixed range for each type of sensor.

This program is intended to simulate real sensors, sending data. 
Use it only for testing purpose, alongside a sensor_alert and/or a sensor_stat to read simulated values.

A shared memory is used to communicate between processes and share sensors data.
Sensor_simulator use the shared memory only in writting mode.
...
$ sudo chmod +x /usr/bin/sensor_simulator
...
```
 ...
 Following theses steps, sensor_simulator program will run automaticaly on boot, along side the sensor_stat and sensor_alerts, with the sensor_simulator launched fisrt.
...
To populate our recipe for sensor-stats, we will import our source files in our recipe to get this architecture:
(assuming that $BUILD_SENSOR_SIMU= /path/to/sensor_stats_sources)

```
../poky/buils$ bitbake-layers create-layer ../meta-sensors
../poky/buils$ cd ../meta-sensors
../poky/meta-sensors$ mv recipes-example recipes-sensor-simu
../poky/meta-sensors$ mkdir recipes-sensor-simu/sensor-simu
../poky/meta-sensors$ mkdir recipes-sensor-simu/sensor-simu/files
../poky/meta-sensors$ cp $BUILD_SENSOR_SIMU/*.* recipes-sensor-stats/sensor-simu/files/
...poky/meta-sensors$ tree recipes-sensor-simu/ -L 3
recipes-sensor-simu/
└── sensor-simu
    ├── files
    │   ├── README.h
...
We will edit our recipe sensor-stats_0.1.bb as follow :

```
SUMMARY = "My Sensor Simu application"
LICENSE = "CLOSED"
PR = "1"
